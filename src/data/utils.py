...
def extract_data_quality_index(dqi):
    import DATA_QUALITY_INDEX
    extract = lambda d: [i for i in d if i != '-']
    return dict(zip(
            [k.replace(" ", "_") for k,v in DATA_QUALITY_INDEX.DATA_QUALITY_INDEX],
...
def extract_regridded_sp(sp):
    ...
    photom_iof_sp_2nm = interpolator(wav_grid_2nm)
    # photom_iof_sp_10nm = interpolator(wav_grid_10nm)
    interpolator = scipy.interpolate.interp1d(wav, iof_sp, kind='linear', fill_value='extrapolate')
    iof_sp_2nm = interpolator(wav_grid_2nm)
    return {'photom_iof_sp_2nm': np_to_sql_arr(photom_iof_sp_2nm),
            'iof_sp_2nm' : np_to_sql_arr(iof_sp_2nm)}

def fov_coord_extractor(sp,shapelyze=True, geoalchemy2ize=True ,srid=4326):
